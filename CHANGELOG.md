...
- **Phase-based tool profiles: server-side `tools/list` filtering** (#782): a client may select a tool profile in its server URL — `http://127.0.0.1:8750/mcp?profile=implement` (explore/edit/verify/publish tooling), `?profile=review` (read + verify + `diff_in_container` + `sandbox_pr_review_write`, no writes and no exec), `?profile=issue` (the minimal issue surface: `issue_view` / `sandbox_issue_write` / `sandbox_pr_review_write`, plus `read_file_range` for the issue body `issue_view` saves into the container — a container is still required, as it is for every sunaba tool) — and the server returns only that subset, cutting the always-loaded tool definitions a session pays for. Every profile carries `get_workflow_guide`. A client with no `profile` parameter gets exactly the previous full list; an unknown (or empty) value fails the request with a message naming the valid profiles rather than silently falling back to everything. Profile membership lives in `src/sunaba/tool_profiles.py`, ships in the wheel like the workflow guide (#728), and is pinned against `workflow_guide.md`'s phase sections by `tests/test_tool_profiles.py`. This is a context-size measure, not a security control: `tools/call` is unfiltered and capability limits stay with the client allowlists and the server's existing checks.

### Changed

- **Implement/review tool profiles now cover the kusabi phase allowlists** (#860): `?profile=implement` gains `sandbox_attach` and `issue_view`, and `?profile=review` gains `sandbox_attach`, `issue_view`, and `sandbox_exec` — the tools the kusabi worker agents are permitted and instructed to use in their implement and review phases. A filtered tool is simply absent from the session, so a profile that under-covers its consumer's allowlist would remove tools silently; the profiles must stay supersets of those allowlists, and `tests/test_tool_profiles.py` now fails if any of the five memberships is removed. Two exclusion rationales that were factually wrong are corrected in the move: attach does not create containers (it is how a worker binds to the container id its brief hands it), and read-only-ness is a client-side capability statement, not a profile property — profiles are a context-size measure, not a security control.

- **fastmcp requirement raised to `>=3.4.4,<4`** (#782): the tool-profile middleware imports `fastmcp.server.middleware` and `fastmcp.server.dependencies.get_http_request`, which are documented from FastMCP 3.4.x. With the old `>=2.0.0` floor and no lockfile, a fresh resolve could pick a 2.x build whose missing import fails at server start, so the floor now matches what the server actually calls.
